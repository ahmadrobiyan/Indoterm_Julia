# ARCHIVED — superseded 2026-09-08, kept for the record, not on any gate path.
#
# An independent V8 implementation written 2026-09-04, never executed. The merge kept the
# other lineage (now test/sensitivity.jl) because that one has actually been run end to end
# and had two defects found and fixed by running it:
#   - it perturbed elasticities AFTER calibration, leaving CES share parameters fitted to the
#     old sigma, which the benchmark gate correctly rejected;
#   - it reported a stalled continuation's residual, which is small at the last accepted step
#     and therefore reads as success.
# Anything worth salvaging here should be ported into test/sensitivity.jl, not revived
# alongside it — two files named for the same gate is how the above went unnoticed.
