...
| V8 Elasticity sensitivity | 🟡 IMPLEMENTED, NOT YET RUN | `test/sensitivity.jl` — Phase A of V8_ELASTICITY_SENSITIVITY.md: 6 elasticity groups x {x0.5, x1.5} + centre = 13 solves against `COALPRICE_REFERENCE`, ~55-85 min. Guards that every key is present, every vector non-zero and every perturbation material, so a sweep cannot silently vary nothing. Deliverable is a centre/min/max/sign-stability table, not a verdict. **Never executed — written in an environment with no Julia runtime; validate with `V8_SMOKE=1` before trusting the full sweep.** |
